...
set _DeploySetParametersFile=%RootPath%IngeniBridge.Server.v.1.0.4.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%IngeniBridge.Server.v.1.0.4.zip" (
echo "%RootPath%IngeniBridge.Server.v.1.0.4.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%IngeniBridge.Server.v.1.0.4.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='Default\ Web\ Site\\App_Data$'
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%IngeniBridge.Server.v.1.0.4.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='Default\ Web\ Site\\App_Data$' -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%IngeniBridge.Server.v.1.0.4.deploy-readme.txt" (
...
start notepad "%RootPath%IngeniBridge.Server.v.1.0.4.deploy-readme.txt"
)
echo =========================================================
goto :eof
